  ...
  return (
    ...
    {showWorldEditor && (
      <WorldEditor tiles={customTiles} onSave={handleSaveWorldTiles} onClose={() => setShowWorldEditor(false)}
        ...
        onSaveNpc={async (positions) => { try { await api.saveNpcPositions(password, positions); setNpcPositions(positions); setMsg("NPC spostati!"); } catch (err: unknown) { setError((err as Error).message); } }} />
    )}
    </>
  );
}
